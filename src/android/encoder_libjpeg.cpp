...
int EncoderLibJpeg::configure(const StreamConfiguration &cfg)
{
	...
	LOG(JPEG, Debug) << "EncoderLibJpeg::configure start";

	if (info.colorSpace == JCS_UNKNOWN) {
		LOG(JPEG, Error) << "EncoderLibJpeg::configure colorspace not supported";
		return -ENOTSUP;
	}
	...
	LOG(JPEG, Debug) << "EncoderLibJpeg::configure width=" << compress_.image_width
					<< " heigth=" << compress_.image_height
					<< " color_space=" << compress_.in_color_space
					<< " numPlanes=" << pixelFormatInfo_->numPlanes();
...
void EncoderLibJpeg::compressNV(const std::vector<Span<uint8_t>> &planes)
{
	...
	LOG(JPEG, Debug) << "EncoderLibJpeg::compressNV start";
			   ...
			   unsigned int quality)
{
	...
	LOG(JPEG, Debug) << "EncoderLibJpeg::encode start #1";
...
int EncoderLibJpeg::encode(const std::vector<Span<uint8_t>> &src,
			   ...
			   unsigned int quality)
{
	LOG(JPEG, Debug) << "EncoderLibJpeg::encode start #2";
